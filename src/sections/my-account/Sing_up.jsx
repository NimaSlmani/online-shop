...
const Sing_up = () => {
    ...
    return (
        <div className="pt-[170px] relative z-20 lg:pt-[60px]">
            <Primary_Title text="Register" theme={theme}/>
            <form className="pt-3 lg:w-3/12">
                <Input placeholder="Username..." theme={theme}/>
                <Input placeholder="Email..." theme={theme}/>
                <Input placeholder="Phonenumber..." theme={theme}/>
                ...
                <p className={`text-14 font-euclidReqular mt-[15px] ${theme === 'light' ? 'text-gray-300' : 'text-black-100'}`}>
                    Ut Enim Ad Minim Veniam, Quis? <span onClick={()=>dispatch(setLogin_page_check(true))} className={theme === 'light' ? 'text-yellow-200 cursor-pointer' : 'text-pruple-200 cursor-pointer'}>Login</span>
                </p>
            </form>
        </div>
    )
